...
class CheckUserView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            cache.set(f"login_{otp}", {"user_id": user.id}, timeout=600)
...
class RegisterRequestView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            cache.set(f"reg_{otp}", user_data, timeout=600)
...
class VerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        # 1. Handle Login
        login_data = cache.get(f"login_{otp}")
        if login_data:
            user = User.objects.get(id=login_data['user_id'])
            cache.delete(f"login_{otp}")
            return Response({"message": "Login Successful", "tokens": get_tokens_for_user(user)})

        # 2. Handle Registration
        reg_data = cache.get(f"reg_{otp}")
        if reg_data:
            # Create user with 'user' role automatically
            user = User.objects.create_user(
                email=reg_data['email'],
                name=reg_data['name'],
                mobile_number=reg_data['mobile_number'],
                role='user' # Critical: Ensures regular users aren't staff
            )
            cache.delete(f"reg_{otp}")
            return Response({"message": "Account Created!", "tokens": get_tokens_for_user(user)}, status=status.HTTP_201_CREATED)
